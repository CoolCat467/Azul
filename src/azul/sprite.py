...
from pygame.event import event_name, Event as PygameEvent
...
class Location(Vector2):
    ...
    __slots__ = ('_rect',)
    def __new__(cls, *args: int | float | complex,
                dtype: type=None) -> Union['Location', Vector2]:# type: ignore
...
class Sprite(ComponentManager, DirtySprite):
    ...
    def __set_location(self, value: Union[list, tuple]) -> None:
    ...
    def __set_image_dims(self, value: tuple[int, int]) -> None:
        ...
        if prev_size == (0, 0) or self.rect.size == (0, 0):
            return
        self.sync_events.append(Event('sprite_image_resized', {
            'prev_size': prev_size,
            'size': self.rect.size,
            'rel': rel,
        }))
    ...
    def __get_image(self) -> Optional[Surface]:
        "Return surface of this sprite"
        return self.__image
    def __set_image(self, image: Optional[Surface]) -> None:
    ...
    image = property(__get_image, __set_image,
                     doc='Image property auto-updating dimentions.')# type: ignore
    ...
    async def raise_queued_sync_events(self, _: Event) -> None:
        ...
        async with trio.open_nursery() as nursery:
            for event in (self.sync_events.popleft() for _ in range(len(self.sync_events))):
...
class ImageComponent(ComponentManager):
    ...
    def __init__(self) -> None:
        ...
        self.set_surface: Optional[int | str] = None
    ...
    def _compute_mask(self, identifier: int | str) -> None:
        ...
        self.__masks[identifier] = mask_from_surface(self.get_image(identifier),
                                                     self.mask_threshold)
    ...
    def get_mask(self, identifier: int | str) -> Mask:
        ...
        if not self.image_exists(identifier):
            raise ValueError(f'No image saved for identifier "{identifier}"')
        if not identifier in self.__masks:
            self._compute_mask(identifier)
...
class AnimationComponent(Component):
    ...
    def __init__(self) -> None:
        ...
        self.controller: Iterator[Optional[str]] = default()
    ...
    async def tick(self, tick_event: Event) -> None:
        ...
        if self.update_every == 0:
            ...
            updates, self.update_mod = divmod(self.update_mod + passed, self.update_every)
...
class MovementComponent(Component):
    ...
    def point_toward(self, position: Iterable) -> None:
        ...
        self.heading = Vector2.from_points(sprite.location, position).normalized()
...
class TargetingComponent(Component):
    ...
    destination = property(__get_destination, __set_destination, doc='Target Destination')
    ...
    def move_destination_time(self, time_passed: float) -> None:
        ...
        travel_distance = min(self.to_destination.magnitude, movement.speed * time_passed)
...
##            self.pressed.clear()
        ...
        async with trio.open_nursery() as nursery:
            for button, pressed in self.pressed.items():
                if not pressed:
                    continue
                nursery.start_soon(self.raise_event, Event('drag', {
                    'pos': event.data['pos'],
                    'rel': event.data['rel'],
                    'button': button,
                }))
...
##    __slots__ = ('groups', 'group_names', 'new_gid', '_timing', '_clear')
    ...
    def get_group(self, gid_name: Union[str, int]) -> Union[sub_renderer_class, None]:

    ...
    def run(self):
        ...
        while self.active and not self.server.closeEvent.isSet():
            try:
                ...
                if self.addr[0] not in self.server.bannedIps:
                    if (
                        not self.recvData
                        or self.recvData == b""
                        or self.server.closeEvent.isSet()
                    ):
                        self.active = False
                    else:
    ...
    def run(self):
        ...
        while server.active and not self.server.closeEvent.isSet():
            if len(self.server.clients) < self.server.maxClients:
                ...
                self.server.clients[newCid] = Client(
                    clientSocket,
                    addr,
                    newCid,
                    self.server,
...
class Server(Thread):
    ...
    def __init__(self, host, port, maxClients, doPrint=False, passwd="rand9"):
        ...
        if len(passwd) >= 4 and passwd[:4] == "rand":
            ...
            choose = [chr(i) for i in range(65, 122)] + [
                str(i) for i in range(10)
            ]
            self.password = "".join(
                [random.choice(choose) for i in range(cnt)],
            )
            ...
            if not trusted:
                if len(cdata) == 2:
                    if cdata[1] == self.password:
                        ...
                        self.clients[fromCid].send_all(
                            "S You are banned from this server for attempted hacking;",
                        )
                ...
                for clientMsg in clientMsgs.split(";"):
                    ...
                    if clientMsg == self.clientLeaveMsg:
                        if fromCid not in clientsToDelete:
                            if fromCid not in deletedClients:

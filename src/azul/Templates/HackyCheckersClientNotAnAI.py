...
def findChange(old, new):
    ...
    twostarts = [i for i in startends if twotidpids[i] == "N"]
...
def update(boardData):
    ...
    try:
        ...
        print(
            "AI: An error occored when trying to send board data to server.",
            file=os.sys.stderr,
        )
        disconnectFromServer()
    else:
...
def turn():
    ...
    if "bye" in functools.reduce(
        ...
        disconnectFromServer()
        return "QUIT"
    if rcvdData == "":  # If the server died
    ...
    while not success:
        ...
        while HOST is None:
            ...
            while True:
                ...
                print(
                    "AI: Please enter the IP Address of the Hacky Checkers Server.",
                )
                ip = input("IP : ")

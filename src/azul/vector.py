...
def simpleop(function: F) -> F:
    ...
    def operator(self, *args: Any):  # type: ignore
        return self.__class__(
            *map(apply, zip(self, *args, strict=True)),
            dtype=self.dtype,
        )
    ...
    def wrapper(
        ...
        if combine == "any":
            ...
            def operator(self: Iterable[bool], rhs: Iterable[bool]) -> bool:
                return any(map(apply, zip(self, rhs, strict=True)))

        else:

            def operator(self: Iterable[bool], rhs: Iterable[bool]) -> bool:
                return all(map(apply, zip(self, rhs, strict=True)))
    ...
    def __init__(
        ...
        *args: V,
        ...
        if dims is None:
            ...
            self.__v = cast(
                Iterable[V],
                self.dtype(list(args) + [0] * (dims - len(args))),
            )

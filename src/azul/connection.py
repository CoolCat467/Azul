...
class BaseWriteSync(ABC):
    ...
    def write_varlong(self, value: int) -> None:
        ...
        for _ in range(10):
            ...
            remaining >>= 7
        raise ValueError(
            f'The value "{value}" is too big to send in a varlong',
        )
...
class BaseConnection:
    ...
    def receive(self, data: BytesConvertable | bytearray) -> None:
        """Raise TypeError, unsupported."""
        raise TypeError(
            f"{self.__class__.__name__} does not support receive()",
        )

    def remaining(self) -> int:
        """Raise TypeError, unsupported."""
        raise TypeError(
            f"{self.__class__.__name__} does not support remaining()",
        )

...
from typing import Union, Any, Generator, IO
from collections import deque
import asyncio
from io import StringIO

from gears import Timer, BaseBot
...
def log_active_exception() -> str:
    ...
    __traceback.print_exception(None, values[1],
                              values[2],
...
class Event:
    ...
    def __repr__(self) -> str:
        ...
        kwargs = ', '.join(key+'='+repr(value) for key, value in self.kwargs.items())
        data = [repr(self.type)]
        if args:
            data.append(args)
        if kwargs:
...
class EventLoop(BaseBot):
    ...
    async def process(self) -> None:
        ...
        if not hasattr(self, 'gears'):
            raise AttributeError('No "gear" attribute defined! Need to subclass BaseBot too!')
        handlers = []
